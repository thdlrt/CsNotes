...
#### 法线变换
- 使用原始矩阵的逆矩阵的转置对法线进行变换 $(\mathbf{M}^{-1})^{\mathbf{T}}$
	- [证明](https://zhuanlan.zhihu.com/p/449976247)
- 如果变换矩阵完全由平移、旋转和均匀缩放操作组合（平移和均匀缩放抖对法线方向没有影响，旋转矩阵是正交矩阵，其逆矩阵的转置就是自身）可以直接使用原变换矩阵 $M$
- 此外如果变换涉及到缩放，需要对法线进行额外的归一化处理
- 求解逆矩阵的开销是很大的，有一些更简单的方法：旋转矩阵（正交矩阵）用转置等价逆；用负参数变换代替原先的变换来得到逆
### 高级
#### 欧拉变换
- 构建一个旋转矩阵，将一个物体指向一个特定方向
- 非常直观和简单的变换方式，分别按照三个轴进行旋转 $\mathbf{E}(h,p,r)=\mathbf{R}_z(r)\mathbf{R}_x(p)\mathbf{R}_y(h)$
- $\mathbf{E}=\begin{pmatrix}\cos r\cos h-\sin r\sin p\sin h&-\sin r\cos p&\cos r\sin h+\sin r\sin p\cos h\\\sin r\cos h+\cos r\sin p\sin h&\cos r\cos p&\sin r\sin h-\cos r\sin p\cos h\\-\cos p\sin h&\sin p&\cos p\cos h\end{pmatrix}$
	- 通过矩阵有办法逆推出每个角度的大小
#### 任意轴旋转
- 先将旋转轴旋转对齐某个坐标轴-执行旋转-回到原先的位置
	- ![image.png|500](https://thdlrt.oss-cn-beijing.aliyuncs.com/undefined20250920121115.png)
	- 为了对齐坐标轴需要先构建一组正交基用于进行旋转操作
#### 四元数
- 具体细节略
- 与欧拉角相比，无锁的问题，并且可以使用球面线性插值来均匀自然的进行旋转，并且性能表现也较好
#### 顶点混合
- 处理连接处的变换，比如手臂连接处的肘关节附近皮肤等如何运动
- 顶点混合允许单个顶点被若干个不同的矩阵所变换，并将这些变换所得到的结果**加权混合**在一起
- 
### 视图变化
