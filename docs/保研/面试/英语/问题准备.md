...
1. chaining: each array bucket points to a linked list of entries that share the same hash index, and new entries are simply added to the list when collisions occur.
2. Open addressing: on the other hand, stores all entries directly in the array. When a collision occurs in open addressing, the algorithm searches for the next empty slot using methods such as linear probing, which checks the next slot sequentially, quadratic probing, which checks slots at increasing quadratic intervals, and double hashing, which uses a second hash function to determine the interval between probes. These methods ensure that the hash table remains efficient even when collisions occur.
### kmp
