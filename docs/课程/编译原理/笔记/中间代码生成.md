...
[[docs/课程/编译原理/作业/p8#^690668|6.6.1]]
#### 布尔表达式
...
- 对于 `if(B)S`
	...
	- 希望能**一趟处理**完成
- 思想：记录 B 中跳转指令 `goto S.next` 的标号，但是先不生成跳转目标，先存储到 list，之后知道目标时统一进行填入
...
- M：用M.instr 记录**下一条指令的标号**
- N：生成 goto 指令坯，N.nextlist 包含该指令标号（除了布尔位置跳转完，**其他位置也可能需要插入跳转 goto**，比如 if true 部分的语句执行完成，需要跳转到整个流程控制语句之后）
